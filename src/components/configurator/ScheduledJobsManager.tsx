...
// Type for scheduled job from the API
...
const scheduleJob = async (jobData: { cronExpression: string; scriptName: string }): Promise<ScheduledJobResponse> => {
  const response = await fetch('/api/start-scheduled-job', {
    method: 'POST',
    headers: {
...
export default function ScheduledJobsManager() {
  ...
  const { data: jobs, isLoading, error, refetch } = useQuery({
    ...
    queryFn: fetchJobs,
    // Refetch every 10 seconds to ensure UI is up-to-date
    refetchInterval: 10000,
    // Keep stale data while fetching
    staleTime: 5000,
  });
  ...
  const handleSchedule = async (jobData: { cronExpression: string; scriptName: string }) => {
    try {
